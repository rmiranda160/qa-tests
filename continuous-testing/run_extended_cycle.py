...
def load_config():
    with open(CONFIG_PATH, 'r') as f:
        config = json.load(f)
    # Add Villa Zocotin if not present
    existing_names = [app['name'] for app in config['applications']]
    if 'Villa Zocotin' not in existing_names:
        villa_app = {
            'name': 'Villa Zocotin',
        ...
        config['applications'].append(villa_app)
        print('Added Villa Zocotin to applications list')
    return config
...
def main():
    ...
    report_path = os.path.join(RESULTS_DIR, f'{timestamp}_extended.json')
    with open(report_path, 'w') as f:
        json.dump(report, f, indent=2)
    
    # Output plain text summary
    print(f"Extended continuous testing completed at {timestamp}")
