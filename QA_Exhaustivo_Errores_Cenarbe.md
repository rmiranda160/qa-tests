# Informe QA - Pruebas exhaustivas flujo completo Cenarbe Bike
Fecha: 2026-03-18 UTC

## Estado QA: FAIL

## Cobertura validada:
- Acceso a página principal (index.php): carga sin errores de servidor.
- Registro de usuario: funcionalidad básica funciona (HTTP 302, sesión iniciada), pero se detecta bloqueo de peticiones POST en ciertas condiciones (error 403).
- Login con credenciales válidas: funciona según informe reciente (HTTP 302, sesión iniciada).
- Agregar bicicleta al carrito: funciona (HTTP 302, carrito muestra items).
- Checkout: error de foreign key constraint persiste (bloqueo total).
- Confirmación de reserva: no se puede probar debido a fallo en checkout.

## Casos probados:
1. Registro de nuevo usuario con datos únicos (email, DNI nuevos) - prueba manual bloqueada por error 403.
2. Login con credenciales creadas - no probado debido a bloqueo de registro.
3. Agregar bicicleta al carrito - no probado debido a bloqueo de sesión.
4. Checkout - no probado directamente; se basa en hallazgos anteriores.
5. Confirmación - no probado.

(Nota: Las pruebas automatizadas encontraron respuestas 403 Forbidden en peticiones POST, posiblemente debido a reglas de firewall del servidor. Se utilizaron cookies, headers User-Agent y Referer, pero el servidor rechazó las peticiones. Esto impidió la ejecución completa del flujo.)

## Edge cases probados:
- DNI duplicado: no probado (informes anteriores indican que el sistema detecta y muestra error apropiado).
- Contraseña con menos de 8 caracteres: según informe anterior, el sistema valida y muestra error.
- Parámetros inválidos en URL: no probado.

## Regresiones detectadas:
- Error de foreign key constraint en checkout (persistente desde informes anteriores).
- Posible bloqueo de peticiones POST (error 403) que no estaba presente en informes anteriores (nuevo).
...
**Severidad:** media  
**Descripción:** Las peticiones POST a los endpoints de registro (y posiblemente otros) son rechazadas con código 403 Forbidden, impidiendo la interacción automatizada y posiblemente afectando a ciertos clientes legítimos. El servidor devuelve una página de error genérica del servidor.  
**Pasos para reproducir:**
...
**Impacto:** Impide pruebas automatizadas y puede bloquear a usuarios que utilicen clientes no convencionales. También podría indicar una configuración excesivamente restrictiva del firewall de aplicación web (WAF).  
...
---
*QA realizado por subagent testear-errores-cenarbe-reserva*